...
- 連絡先は X `@hdsh0428`。GitHub は運用上 `hideshi`。公開 HTML のナビ・フッター・JSON-LD に GitHub は出さない。記事本文がリポジトリを扱うときだけリンクしてよい。
...
8. 読者が一語で止まらないようにする。英語の直訳、論文の圧縮、業界符丁は、定義なしに本文へ出さない。言い換えを本文にする。原文の段階名（tutor 等）を残すなら、その直前で日本語の動きを書く。
9. 素材の主張と論文の語彙を同一視しない。素材は素材の文で置き、重ねは後段の論文の節で近い／違うと書く。観察の節に論文の祖先（Wood）や論文の術語（答え責任）を出さない。三相の一文だけ直して、前後に論文を残さない。
10. ずらしで一度受けた比喩を、後半で別の意味のまま再利用しない。使うならその場で言い換える。
11. **入口は動作文にする。** title / description / X / 導入の2段落は、本文を読んでいない人の入口である。ここで比喩を解かせない。止まれば、名前を消して「誰が・何を・いつ」の動作に戻す。「〜ということですか？」は入口の失敗信号である。説明を足して守るのではなく、入口を直す。

## 読み手の一文テスト

カード、検索スニペット、タイムライン、導入は、同じ入口である。公開前に title + description + 導入2段落 + X を、本文なしで声に出す。止まれば直す。

1. **主語と動作を先に書く。** 「コードが速く出る」ではなく「AIでコードがすぐ生成できるようになる」。「席が先」ではなく「慣れる前に設計を任される」。
2. **title / description / X に、本文で解く比喩を置かない。** 椅子、飛び級、前倒し、足場など、段落が要る語は入口に使わない。入口は願い → ずらし → 残す線を、日常の動詞で書く。
3. **比喩は動作のあとで、一度だけ名付ける。** OGP の絵に椅子があっても、title を「席が先」にしない。本文で使うなら、動作を書いたあとに「これを〜とも書く」と一度だけ。
4. **サイト名の「足場」と本文の支援を混用しない。** 本文では先に「一時的な支援」と書き、論文の節で足場と接続する。
5. **テスト:** 入口だけ渡した人が、「誰が、何を、いつ任されているか／何を見るか」を言い返せるか。言えなければ直す。

title はずらしが一文で見える形（なぜ〜か、〜する前に、〜に似ている）。ただしその一文が、比喩なしで願いとずらしを含むこと。description も同じ。便利さの片側だけにしない。

2026-08-29「設計を任せる」で止まった語と直し（入口の実例）:

| 出していた語 | 読者に届かなかった意味 | 入口・本文の言い換え |
|---|---|---|
| 任ること | 任せる、の途中形。にん／まか で止まる | 任せること |
| コードが速く出る | AIの生成が速くなった | AIでコードがすぐ出るようになる |
| 席が先／席の前倒し | 慣れる前に設計を任される | 慣れる前に任されている |
| 飛び級／跳ぶ | 能力が一段上がった | 成長が早まった |
| 足場（入口） | 一時的な支援 | 支援。論文の節で足場と接続 |
| 切る位置 | 障害の切り分け、最初に疑う場所 | どこから疑うか |
| 赤旗 | red flag、ツールの警告 | ツールが出した警告 |
| 出荷 | マージして出す量 | 出したコードの量 |
| 経験の前に跳ぶ | 求められる仕事が先に来る | 実装で慣れる前に、設計判断を求められる |
| エッジ | edge cases | 例外の扱い |
| 個人の不足 | 能力が足りない人のせい | そのジュニアが未熟だから、と決めつける |
| 頭が足りない | 地頭論文への圧縮。罵りに読める | その人の能力の問題に読み替える |
| LeadDev | 媒体名 | 初出は役割＋リンク。後は「同記事」 |
| 届く（Wood） | 一人でできるようになる | 自分で答えられる範囲 |
| 三相＝Woodの一時性 | 素材を論文定義へ置換 | 三相は支援を明示し判断が見えたら緩める。Wood そのものではない |
...
| `_layouts/default.html` | `{% seo %}` → `{% feed_meta %}` → `head-twitter.html`。`name="twitter:image"` は SEO の `property` より後。記事・about のサイト名は `h1` にしない。GitHub の View / Profile / メンテナ / テーマ作者リンクは出さない |
...
文面はカード用の要約ではなく、タイムラインで読める短文にする。1文は1つの動き（願い → ずらし → 残す線）。であるの畳みかけや、description のほぼ全文貼りは避ける。入口の一文テストを通す。席・足場・跳ぶなど、カードの外で解く比喩は置かない。AIが主語になる話なら、AIと書く。
...
2026-08-27 ドキュメント形式記事で通した文面:

```
記事を書きました。
文書を一つの形式に揃えたくなるのは、書く側と機械の負担を減らしたいからだと思う。それを個人の書き方の問題にすると、提出先の決まりが見えなくなる。形式は、誰が答えられるかの配置である。

https://hideshi.github.io/blog/2026/08/27/document-format-and-who-can-answer/
```

2026-08-29 設計を任せる記事。入口を動作文にした文面:
...
title: "..."
date: YYYY-MM-DD
description: "120字前後。検索と X 用。比喩を解かずに願いとずらしが読めること。"
image:
